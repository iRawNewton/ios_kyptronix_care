import 'dart:convert';
import 'package:ios_kyptronix_care/screens/createuser/developer/createdev.dart';
import 'package:ios_kyptronix_care/screens/widgets/widgets.dart';
...
import '../dashboard/admin/detailsdialog.dart';
import '../dashboard/client/colorwidget.dart';
import 'package:url_launcher/url_launcher.dart';

var tempUrl = AppUrl.hostingerUrl;

class MyDevList extends StatefulWidget {
  const MyDevList({super.key});

  @override
  State<MyDevList> createState() => _MyDevListState();
}

class _MyDevListState extends State<MyDevList> {
  ...
  searchDevList(devId) async {
    var response =
        await http.post(Uri.parse('$tempUrl/dev/searchdev.php'), body: {
      'searchValue': devId.text,
    });
  ...
  // delete function
  deleteData(devId) async {
    var response =
        await http.post(Uri.parse('$tempUrl/dev/deletedev.php'), body: {
      'searchValue': devId.toString(),
    });
    if (response.statusCode == 200) {
      setState(() {
        getFutureMethod = getDevList();
        showCard = true;
      });
    } else {}
  }
                                                ...
                                                child: Column(
                                                  children: [
                                                    ...
                                                    Align(
                                                      ...
                                                      child: Text(
                                                        'Username: ${data[index]['cli_username']}',
                                                        style: TextStyle(
                                                          fontFamily: 'fontTwo',
                                                          fontSize: 18,
                                                          fontWeight:
                                                    ...
                                                    Align(
                                                      alignment:
                                                          Alignment.centerLeft,
                                                      child: Text(
                                                        'Password: ${data[index]['cli_password']}',
                                                        style: TextStyle(
                                                          fontFamily: 'fontTwo',
                                                          fontSize: 18,
                                                          fontWeight:
