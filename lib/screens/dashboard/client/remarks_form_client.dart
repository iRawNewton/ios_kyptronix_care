...
class MyClientRemarksForm extends StatefulWidget {
  const MyClientRemarksForm({super.key, required this.projectID});

  final String projectID;

  @override
  State<MyClientRemarksForm> createState() => _MyClientRemarksFormState();
}

class _MyClientRemarksFormState extends State<MyClientRemarksForm> {
  // variables
...
// ! add manager email too in cc and find which manager`
    // sentEmail(devId, context);

    // end dev id
    var response = await http
        .post(Uri.parse('$tempUrl/remarks/insertClientRemarks.php'), body: {
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.pushReplacement(context,
          MaterialPageRoute(builder: (context) => const MyClientDashboard()));
    } else {}
  }
